...
<script>
  ...
  var sim = d3.forceSimulation()
      .force("x", d3.forceX(150).strength(0.005));
  ...
  function dataViz(data) {
    ...
    function mouseOverFunction(p) {

      var child = d3.select(this);
      
      child.transition().duration(1500).attr("opacity", 1);
      
      var parent = child.select(function() {
        var element = this;
        while(!d3.select(element).classed("gCircle"))
          element = element.parentElement;
        return element;
      });
      
      parent.raise().append("text").text(p.id).attr("transform", "translate(15, -15)");
    }

    function mouseOutFunction(p) {
      
      var child = d3.select(this);
      
      child.transition().duration(1500).attr("opacity", p => p.alpha);
      ...
      parent.select("text").remove();
      
      <!-- d3.select(this) -->
      <!--   .selectAll("circle") -->
      <!--   .attr("opacity", p => p.alpha) -->
